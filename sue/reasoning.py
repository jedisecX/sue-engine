...
INTENTS = ("reflect", "question", "connect", "disagree", "admit", "play", "observe")
...
class Reasoner:
    ...
    def consider(self, parsed: Parsed, state: State, memory: Memory) -> tuple[list[Thought], Thought]:
        retrieved = memory.retrieve(parsed.content or parsed.tokens, k=4)
    ...
    def _spawn(self, parsed: Parsed, state: State, retrieved: list[Trace]) -> list[Thought]:
        ...
        if hint in weak and state.topic:
            topic = state.topic
        else:
            topic = hint or state.topic or "this"
    ...
    def maybe_new_goal(self, parsed: Parsed, state: State) -> Goal | None:
        ...
        templates = (
            f"understand {seed}",
            f"find what {seed} sits next to",
            f"test whether {seed} repeats",
        )
